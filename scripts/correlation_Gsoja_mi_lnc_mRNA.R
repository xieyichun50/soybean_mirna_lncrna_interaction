...
exp.mRNA<-read.csv(file = "run/Gsoja/Gsoja_mRNA_filter_gene_count_matrix.csv", 
                   header = TRUE)
DEG.mrna<-read.table("run/Gsoja/DEGlist.Gsoja.txt", header = TRUE)
...
exp.miRNA<-read.csv(file = "run/Gsoja_miRNA_soybean_salt_smallRNA.miRBase_22.1_mature.gma.uniq_seq.norm.csv",
                    header = TRUE)
...
exp.lncRNA<-read.csv(file = "run/Gsoja_lncRNA_NPC_TS_Cufflinks_FPKM4R.csv",
                     header = TRUE)
DEG.lncRNA<-read.table("run/Gsoja_NPC_TS_Cuffdiff_time_series_EI_SR_R_LI_SR_L_stranded_FPKM4R.txt", header = TRUE)
...
write.table(corr.filter.merge, 
            file = "PCC_Gsoja_miRNA_lncRNA_corr.txt", 
            sep = '\t', row.names = FALSE,
            quote = FALSE)
...
write.table(corr.filter.merge, 
            file = "PCC_Gsoja_mRNA_lncRNA_corr.txt", 
            sep = '\t', row.names = FALSE,
            quote = FALSE)
...
write.table(corr.filter.merge,
            file = "PCC_Gsoja_mRNA_miRNA_corr.txt", 
            sep = '\t', row.names = FALSE,
            quote = FALSE)
